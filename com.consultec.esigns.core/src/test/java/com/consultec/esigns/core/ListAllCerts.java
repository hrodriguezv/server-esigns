...
/**
 * This example will list all certificates stored in all available MS CAPI system (certificate)
 * stores.
 */
public class ListAllCerts {

  /** The Constant logger. */
  private static final Logger logger = LoggerFactory.getLogger(ListAllCerts.class);
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      for (Enumeration<String> e = ks.aliases(); e.hasMoreElements();) {
        ...
        logger.info("Certificate friendly name: " + JCAPIUtil.getCertificateFriendlyName(alias));
        logger.info("\nCertificate: \n"
            + ((X509Certificate) ks.getCertificate(alias)).getSubjectX500Principal().getName());
        X500Name x500name =
            new JcaX509CertificateHolder((X509Certificate) ks.getCertificate(alias)).getSubject();
        if (x500name.getRDNs(BCStyle.CN).length > 0) {
          RDN cn = x500name.getRDNs(BCStyle.CN)[0];
          logger.info(IETFUtils.valueToString(cn.getFirst().getValue()));
        }
        i++;
      }
